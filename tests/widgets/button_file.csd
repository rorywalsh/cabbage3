...
filebutton bounds(16, 12, 117, 30) channel("loadfile"), text("Load Sample"), corners(5)
button bounds(146, 12, 80, 30) channel("play"), text("Play", "Stop"), corners(5)
button bounds(242, 12, 117, 30) channel("randomSpeed"), text("Random Speed"), corners(5)

texteditor bounds(18, 256, 341, 208) channel("infoText"), readOnly(1), wrap(1)
gentable bounds(18, 76, 342, 157), channel("gentable1"), tableNumber(1), colour(147, 210, 0)
...
instr 1
    ...
    SText  = "This instrument shows an example of how file buttons can be used in Cabbage. A file button will launch a browser dialogue when clicked. The user selects a file or directory, depending on the mode. When they click Ok, it will return the name of the file or directory they just selected. In the example above, a gentable widget is used to display the selected file"
    cabbageSet "infoText", "text", SText

    ;load sample to soundfiler - and dismiss wanring if it is showing
    SFilename, kLoadFile cabbageGetValue "loadfile"
    cabbageSet kLoadFile, "gentable1", "file", SFilename
...
instr SamplePlayback
    ...
    if strlen(SFile) == 0 then    
        ...
        if filenchnls(SFile) == 1 then
            a1 diskin2 SFile, kRandomSpeed, 0, 1
            outs a1, a1
        else
