...
macro(fetch_github_dependency name)
    ...
    cmake_parse_arguments(ARG "USE_GIT;USE_SSH" "GIT_REPOSITORY;GIT_TAG;URL;URL_MD5;SOURCE_DIR" "" ${ARGN})
    message(TRACE "Macro: fetch_github_dependency(${name}) - ARG_USE_GIT = ${ARG_USE_GIT}")
    message(TRACE "Macro: fetch_github_dependency(${name}) - ARG_USE_SSH = ${ARG_USE_SSH}")
    ...
    if(${ARG_USE_GIT})
        if(DEFINED IS_CABBAGE_CI_BUILD AND ${IS_CABBAGE_CI_BUILD})
        ...
        if(EXISTS "${ARG_SOURCE_DIR}" AND NOT EXISTS "${ARG_SOURCE_DIR}/.git")
            message(TRACE "Macro: fetch_github_dependency(${name}) - Removing ${ARG_SOURCE_DIR} to re-fetch the dependency from the Git repository.")
        ...
        if(${ARG_USE_SSH})
            string(REGEX REPLACE "^https://github.com/(.+).git$" "git@github.com:\\1.git" ARG_GIT_REPOSITORY "${ARG_GIT_REPOSITORY}")
        endif()
        ...
        if(NOT ARG_URL)
            # Convert the git url and commit/tag to the .tar.gz url automatically generated by GitHub for each commit/tag.
            string(REGEX REPLACE "^https://github.com/(.+).git/(.*)$" "https://codeload.github.com/\\1/tar.gz/\\2" ARG_URL "${ARG_GIT_REPOSITORY}/${ARG_GIT_TAG}")
        endif()
        ...
        if(EXISTS "${ARG_SOURCE_DIR}/.git")
            message(TRACE "Macro: fetch_github_dependency(${name}) - Removing ${ARG_SOURCE_DIR} to re-fetch the dependency from the Github .tar.gz.")
            execute_process(COMMAND ${CMAKE_COMMAND} -E remove_directory "${ARG_SOURCE_DIR}")
            execute_process(COMMAND ${CMAKE_COMMAND} -E remove_directory "${FETCHCONTENT_BASE_DIR}/${name}-build")
            execute_process(COMMAND ${CMAKE_COMMAND} -E remove_directory "${FETCHCONTENT_BASE_DIR}/${name}-subbuild")
        endif()
