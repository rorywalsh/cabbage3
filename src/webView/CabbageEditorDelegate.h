...
class CabbageEditorDelegate : public iplug::IEditorDelegate, public iplug::IWebView
{
  ...
  public:
    ...
    void SendParameterValueFromDelegate(int paramIdx, double value, bool normalized) override
    {
        ...
        str.SetFormatted(
            mMaxJSStringLength,
            "if (typeof sendParameterValueFromEditor !== \"undefined\") sendParameterValueFromEditor(%i, %f);",
            paramIdx, value);
        EvaluateJavaScript(str.Get());
    }
    ...
    void SendMidiMsgFromDelegate(const iplug::IMidiMsg &msg) override
    {
        std::string message = cabbage::StringFormatter::format(R"(
          ...
          document.dispatchEvent(customEvent);
                                                               ...
                                                               std::to_string(msg.mStatus), std::to_string(msg.mData1),
                                                               std::to_string(msg.mData1));
    ...
    void OnMessageFromWebView(const char *jsonStr) override
    {
        ...
        //        }
        //
        //        SendArbitraryMsgFromUI(json["msgTag"], json["ctrlTag"], static_cast<int>(base64.size()),
        //        base64.data());
        //    }
        else if (command == "midiMessage")
        {
            auto jsonContent = nlohmann::json::parse(incomingJson["obj"].get<std::string>());
            iplug::IMidiMsg msg{0, jsonContent["statusByte"].get<uint8_t>(), jsonContent["dataByte1"].get<uint8_t>(),
                                jsonContent["dataByte2"].get<uint8_t>()};
            SendMidiMsgFromUI(msg);
        }
    }

    void OnMidiMsgUI(const iplug::IMidiMsg &msg) override { cabAssert(false, "false"); }
