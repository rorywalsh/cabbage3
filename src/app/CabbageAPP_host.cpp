...
void IPlugAPPHost::UpdateSettings()
{
    ...
    settingsJSON["currentConfig"]["audio"]["bufferSize"] = mState.mBufferSize;
...
bool IPlugAPPHost::TryToChangeAudio()
{
    ...
    //      RtAudio::DeviceInfo info = mDAC->getDeviceInfo(d);
    //      cabbage::logInfo << "Device " << info.name;
    //    }
    ...
    if (inputID == -1)
    {
        if (defaultInputId > -1)
        {
            ...
            cabbage::logInfo << mDAC->getDeviceInfo(inputID).name.c_str();
            mState.mAudioInDev.Set(mDAC->getDeviceInfo(inputID).name.c_str());
        }
        else
